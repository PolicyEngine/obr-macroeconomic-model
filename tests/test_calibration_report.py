...
def test_scorecard_excludes_passthroughs_from_accuracy_denominator(monkeypatch):
    ...
    columns = sorted(
        {entry[0] for rows in calibration_score.PANEL.values() for entry in rows}
    )
    ...
    assert next(row for row in rows if row["variable"] == "CONS")["status"].startswith(
        "passthrough"
    )
...
def test_scorecard_states_what_it_does_not_validate(monkeypatch):
    ...
    columns.append("RPIGR")
    data = pd.DataFrame(
        np.ones((len(periods), len(columns))), index=periods, columns=columns
    )
