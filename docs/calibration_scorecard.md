...
**EFO vintage: March 2026** (re-anchored from the November 2025 EFO; the
October-2025 model code is unchanged).

> **Complete-snapshot revision (this vintage).** The figures above follow the
> repair of the ONS pull: the venv had no CA bundle, every fetch failed with
> `CERTIFICATE_VERIFY_FAILED`, and `ons_pull` logs per-series failures and
> carries on — so the shipped snapshot held 347 series instead of 355. With the
> bundle in place the snapshot is complete, skipped equations fall 86 → 79, and
> the `OLPEx → OLPE → NFWPE` chain unfreezes (household net financial wealth was
> pinned at its 2026Q3 value, which dragged the consumption error-correction
> term down). Every computed line improved except the trade balance, and the two
> "off" lines cleared. The superseded figures were Real GDP 4.48%, Consumption
> 7.49%, Business investment 15.73%, Unemployment 1.01pp, RPI 1.71pp, Household
> income 6.27%, Real household income 6.03%, Company profits 63.29% and the
> current account 3.60% of GDP.
...
merged into a March-2026 EFO baseline (October-2025 model code); where the
...
|---|--:|---|
| Real GDP | 4.22% | computed — fair |
| Consumption | 7.06% | computed — fair |
| Business investment | 11.85% | computed — poor |
| Employment | 0.00% | computed — **trivial identity** |
| Unemployment rate | 0.85pp | computed — fair |
| RPI inflation | 1.48pp | computed — poor |
| Household income | 5.53% | computed — fair |
| Real household income | 5.29% | computed — fair |
| Company profits | 23.32% | computed — poor |
| Current account | 2.74% of GDP | computed — poor |
| Trade balance | 0.76% of GDP | computed — fair |
...
bitwise identical, business investment (11.85%) and Company profits (23.32%)
...
- Of the 11 computed: 1 is a trivial identity, 6 are fair, 4 are poor, and **none is off**.
...
current account is *off* (4.17% of GDP) once de-seeded — driven by a net
...
the current account are *poor/off* but no longer diverging. Company profits
(FYCPR) and the current account (CB) are *off* on the March-2026 vintage;
FYCPR's residual is dominated by the documented OSHH
